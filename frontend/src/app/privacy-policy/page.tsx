...
export default function PrivacyPolicyPage() {
  const [data, setData] = React.useState({ title: "Privacy Policy", content: "" });
  const [loading, setLoading] = React.useState(true);

  const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";

  React.useEffect(() => {
    fetch(`${API_URL}/api/v1/page/privacy-policy?nocache=1`)
      .then((res) => res.json())
      .then((resData) => {
  ...
  function parseMarkdown(text: string) {
    ...
    return text.split("\n").map((line, idx) => {
      ...
      if (line.startsWith("- ")) {
        return <li key={idx} className="list-disc pl-4 ml-4 text-zinc-600 font-semibold mb-1.5 leading-relaxed">{line.replace("- ", "")}</li>;
      }
      if (line.trim() === "") {
