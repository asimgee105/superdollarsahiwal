...
export default function CareersPage() {
  const [data, setData] = React.useState({ title: "Careers", content: "" });
  const [loading, setLoading] = React.useState(true);

  const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";

  React.useEffect(() => {
    fetch(`${API_URL}/api/v1/page/careers?nocache=1`)
      .then((res) => res.json())
      .then((resData) => {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white border border-zinc-150 rounded-2xl p-6 sm:p-8 shadow-3xs space-y-6">
          <div className="flex items-center gap-3 border-b border-zinc-100 pb-5">
            ...
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-[#f51c50]">Work With Us</span>
